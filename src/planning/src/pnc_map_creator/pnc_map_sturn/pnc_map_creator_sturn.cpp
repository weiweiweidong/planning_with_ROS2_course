...
namespace Planning
{
    ...
    PNCMap PNCMapCreatorSTurn::create_pnc_map() // 生成地图
    {
        ...
        draw_arc(M_PI_2, 1.0);  // 逆时针90度
        draw_arc(M_PI_2, -1.0); // 顺时针90度
    ...
    void PNCMapCreatorSTurn::draw_straight_x(const double &length, const double &plus_flag, const double &ratio) // 沿x轴画直线
    {
        double len_tmp = 0.0;
    ...
    void PNCMapCreatorSTurn::draw_arc(const double &angle, const double &plus_flag, const double &ratio) // 画弧线。正方向为逆时针，负方向为顺时针，angle为总角度
    {
